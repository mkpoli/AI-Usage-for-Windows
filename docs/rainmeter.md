...
and current value, and a countdown to its reset. The headline limit is the
provider's primary bar, which is Session for Claude.
...
Percentages describe how much of a limit has been spent. The **Usage Mode**
setting in the app changes the app's own display and leaves these values alone;
a skin that wants remaining headroom subtracts from 100.
